...
bool readVertex(const s21::s21_Graph& graph, const std::string& prompt,
                int& vertex) {
    std::cout << prompt;
    if (!(std::cin >> vertex) || vertex < 1 ||
        vertex > graph.get_vertex_count()) {
    ...
    while (option != 0) {
        ...
        try {
            if (option == 1) {
                ...
                graph = loadedGraph;
                graphLoaded = true;
                std::cout << "Graph loaded: " << graph.get_vertex_count()
                          << " vertices.\n";
                ...
                std::vector<int> route = option == 2
                    ? s21::s21_GraphAlgorithms::BreadthFirstSearch(
                          graph, startVertex)
              : s21::s21_GraphAlgorithms::DepthFirstSearch(
                  graph, startVertex);
            printRoute(route);
            } else if (option == 4) {
                int startVertex;
                int endVertex;
                if (!readVertex(graph, "Start vertex: ", startVertex) ||
                    !readVertex(graph, "End vertex: ", endVertex)) {
                    continue;}
                int distance =
                    s21::s21_GraphAlgorithms::GetShortestPathBetweenTwoVertices(
                        graph, startVertex, endVertex);
                std::cout << "Shortest path length: " << distance << '\n';
            } else if (option == 5) {
                printMatrix(
                    s21::s21_GraphAlgorithms::getShortestPathsMatrix(graph));
            } else if (option == 6) {
                printMatrix(
                    s21::s21_GraphAlgorithms::GetLeastSpanningTree(graph));
            } else if (option == 7) {
                s21::TsmResult result =
                    s21::s21_GraphAlgorithms::SolveTravelingSalesmanProblem(
                        graph);
                ...
                for (int i = 0; i< rounds; i++){
                    s21::TsmResult result =  s21::s21_GraphAlgorithms::SolveTravelingSalesmanProblem(
                        graph);
                    printSalesman(result, graph.get_vertex_count());
                }
                ...
                for (int i = 0; i< rounds; i++){
                    s21::TsmResult result =  s21::s21_GraphAlgorithms::NaiveSalesman(
                        graph);
                    printSalesman(result, graph.get_vertex_count());
                }
                ...
                start = high_resolution_clock::now();
                for (int i = 0; i< rounds; i++){
                    s21::TsmResult result =  s21::s21_GraphAlgorithms::NearestNeighbourSalesman(
                        graph);
                    printSalesman(result, graph.get_vertex_count());
                }
                stop = high_resolution_clock::now();
