...
void expectClosedRoute(const s21::TsmResult& result, int vertexCount) {
    ...
    EXPECT_TRUE(std::all_of(visited.begin(), visited.end(), [](bool value) {
        return value;
    }));
}
...
TEST(GraphAlgorithms, BreadthFirstSearch) {
    ...
    EXPECT_EQ(s21::s21_GraphAlgorithms::BreadthFirstSearch(graph, 1),
...
TEST(GraphAlgorithms, DepthFirstSearch) {
    ...
    EXPECT_EQ(s21::s21_GraphAlgorithms::DepthFirstSearch(graph, 1),
              (std::vector<int>{1, 3, 4, 2}));
}
...
TEST(GraphAlgorithms, ShortestPath) {
    ...
    EXPECT_EQ(
        s21::s21_GraphAlgorithms::GetShortestPathBetweenTwoVertices(graph, 1, 4),
        4);
}
...
TEST(GraphAlgorithms, ConnectivityAndCompleteness) {
    auto graph = makeWeightedGraph();
    s21::s21_Graph disconnected({
        {0, 1, 0},
        {1, 0, 0},
        {0, 0, 0}
    });
    s21::s21_Graph complete({
        {0, 1, 1},
        {1, 0, 1},
        {1, 1, 0}
    });
...
TEST(GraphAlgorithms, SalesmanReturnsClosedRoute) {
    ...
    s21::TsmResult result =
        s21::s21_GraphAlgorithms::SolveTravelingSalesmanProblem(graph);
...
TEST(GraphAlgorithms, NaiveSalesmanFindsShortestRoute) {
    ...
    const int vertexCount = graph.get_vertex_count();
    s21::TsmResult result =
        s21::s21_GraphAlgorithms::NaiveSalesman(graph);
...
TEST(GraphAlgorithms, NearestNeighbourSalesmanReturnsValidRoute) {
    ...
    const int vertexCount = graph.get_vertex_count();
    s21::TsmResult result =
        s21::s21_GraphAlgorithms::NearestNeighbourSalesman(graph);
